/**
 * @file writer.c
 * @brief Creates directories as needed and writes a string to a file.
 *
 * This program accepts a file path and a string as command-line arguments.
 * Missing parent directories are created automatically before writing the
 * specified string to the target file. Diagnostic and error information
...
/**
 * @brief Create all directories in the specified path.
 *
 * Traverses the given path and creates each directory level that does not
 * already exist. A temporary copy of the input string is modified during
 * processing.
 *
 * Example:
 *     Input: "/tmp/a/b/c"
 *     Creates: /tmp, /tmp/a, /tmp/a/b, /tmp/a/b/c
 *
 ...
 *
 * Validates the command-line arguments, creates any missing directories,
 * and writes the specified string to the target file.
 *
